...
Daily adjusted closing prices are downloaded from Yahoo Finance via the yfinance Python library for all tickers present in the cleaned filings, with a start date of June 2014 (to support pre-event estimation windows) and an end date of January 2025. The resulting price panel covers 2,665 trading days across 3,441 tickers. Tickers that failed to download (delisted, renamed, or data unavailable) are logged; this is a known source of potential survivorship bias and is discussed in Section 7. SPY adjusted close prices are downloaded separately over the same period.

Market capitalizations are obtained from Yahoo Finance using the most recent available value at the time of data collection. This is not a point-in-time measure; it reflects current market cap rather than market cap at the time of each event. This look-ahead bias in the universe filter is documented as a limitation in Section 7.

**[DAY 13 TODO: Insert Table 1 — Sample Construction Panel here. Rows: raw filings, after price filter, after dedup, unique tickers with prices, cluster events detected, after market cap filter, after overlap dedup, final sample. Columns: count, description.]**

After applying the cluster detection algorithm (Section 4), universe filters (transaction price ≥ \$5, market cap ≥ \$300M), and 60-day overlap deduplication, the final sample contains 2,664 cluster events across 1,030 unique tickers. Event frequency is elevated in 2020, consistent with broad insider buying during the COVID-19 market disruption. The median market capitalization of event firms is \$1.57 billion.
...
**Signal detection.** For each ticker, I scan all cleaned filings chronologically using a greedy sliding-window algorithm. Starting from the earliest filing, I define a 30-calendar-day forward window and count the number of unique insider names with at least one open-market purchase filing in that window. If the count reaches 3 or more, a cluster event is recorded. The event date is set to the latest (most recent) filing date among the insiders in that cluster window. The scan then advances past the end of the current window and resumes. Tickers with fewer than 3 total unique insiders across the full sample are skipped entirely.
...
**Overlap deduplication.** For each ticker, if multiple cluster events occur within 60 calendar days of each other, only the first event is retained. This prevents the same underlying information episode from generating multiple correlated observations.
...
where $r_{\text{stock},t}$ and $r_{\text{SPY},t}$ are daily returns on trading day $t$. This covers $W + 1$ trading days (days 0 through $W$ inclusive). If any daily return in the window is missing (NaN), the event's CAR for that window is set to NaN and excluded from the statistical test. Four horizons are evaluated: $W \in \{1, 5, 30, 60\}$. The primary pre-registered horizon is CAR(0, 30).

This market-adjusted return approach follows standard event-study practice as described in MacKinlay (1997), with SPY serving as the market benchmark.
...
The primary market-adjusted results across all four pre-registered horizons are presented in Table 2.
...
*Notes: Significance assessed at Bonferroni-adjusted threshold of 0.0125 (= 0.05/4). Hit rate is the percentage of events with CAR > 0. N varies across horizons due to missing price data at longer windows.*
...
Several features of the results merit comment. First, abnormal returns are concentrated in the first few days after the event: the mean CAR(0, 1) of +1.22% represents the majority of the CAR(0, 5) of +1.47%. This is consistent with a rapid information incorporation story, where the market responds quickly once the last cluster filing becomes public. Second, the gap between mean and median widens at longer horizons, indicating right-skew in the CAR distribution — a minority of events produce large positive returns that pull the mean above the median. Third, hit rates decline from 60.6% at the 1-day horizon to 51.8% at 30 days, suggesting that while the average effect is positive, individual event outcomes become noisier over longer windows.

**[DAY 13 TODO: Add Figure 1 reference here — CAR curve (car_curve.png). Interpret: show the time-path of mean CAR across horizons with confidence bands. Note the steep initial rise and flattening.]**

**[DAY 13 TODO: Add Figure 2 reference here — CAR(0,30) distribution (car_dist_30d.png). Interpret: right-skewed distribution, majority of mass near zero, long right tail driving the mean above the median.]**

**[DAY 13 TODO: Add Figure 3 reference here — Yearly CAR(0,30) (car_by_year.png). Interpret: signal present in most individual years, no single year dominates.]**
...
**[DAY 13 TODO: Insert Table 4 — Signal Variant Results here. Columns: variant description, N events, mean CAR(0,30), t-stat, p-value. Rows: baseline (3/30), loose (2/30), strict (5/30), short window (3/14), long window (3/60). Source: results_robustness_signals.md.]**

All variants use the same universe filters, overlap rules, and return computation as the primary analysis.
...
**[DAY 13 TODO: Insert Table 5 — Subsample Results here. Columns: subsample, N, mean CAR(0,30), t-stat, p-value. Rows: full sample, 2015–2019, 2020–2024, large-cap (top 75%). Source: results_robustness_subsamples.md.]**

The primary CAR(0, 30) is statistically significant in both time periods (+1.63%, p = 3.5e-05 for 2015–2019; +1.67%, p = 2.0e-04 for 2020–2024) and in the large-cap subsample (+1.95%, p = 1.4e-08). The signal is not driven by a single subperiod or by small-cap effects.
