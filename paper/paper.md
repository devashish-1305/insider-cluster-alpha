...
This paper tests whether clustered insider open-market purchases predict positive abnormal stock returns in U.S. equities. I define a cluster event as 3 or more unique corporate insiders filing open-market purchases for the same stock within a rolling 30-calendar-day window, with the event date set to the last filing date in the cluster. All methodological choices — signal definition, universe filters, return construction, and statistical tests — are pre-registered before analysis. Applying this framework to SEC Form 4 filings from 2015 to 2024, I identify 2,664 cluster events across 1,030 unique firms. At the pre-registered primary horizon of 30 trading days, mean cumulative abnormal return (market-adjusted) is +1.65% (N = 2,565; t = 5.42, p < 1e-7), exceeding the Bonferroni-adjusted significance threshold. Abnormal returns are concentrated in the first trading day (+1.22%), consistent with rapid market incorporation of newly disclosed insider information. The result is robust to CAPM risk adjustment (30-day CAR = +2.99%; t = 9.74), alternative signal calibrations (2+ to 5+ insiders, 14- to 60-day windows), and subsample splits by time period and market capitalization. The findings suggest that temporal clustering of insider purchases conveys incremental information beyond what individual insider trades reveal, consistent with cluster events filtering for high-conviction, opportunistic insider behavior.
...
This study makes three contributions to the insider trading literature. First, it defines and tests a specific cluster purchase signal — 3 or more unique insiders buying within a 30-day window — not previously examined as a distinct predictor of abnormal returns. Second, it employs a fully pre-registered design that eliminates specification search and ensures confirmatory rather than exploratory inference. Third, it provides comprehensive robustness evidence across CAPM risk adjustment, alternative signal calibrations, and temporal and size-based subsample splits.
...
A gap in the existing literature is the systematic evaluation of clustered insider purchases as a distinct signal. While Lakonishok and Lee (2001) aggregate insider activity at the firm level and Cohen, Malloy, and Pomorski (2012) classify individual trades, no prior study to my knowledge defines and tests a specific cluster rule — multiple unique insiders purchasing within a fixed calendar window — using a pre-registered methodology. This paper fills that gap by formally defining a cluster rule and evaluating its predictive power using a pre-registered event-study framework. While prior work shows that insider purchases contain predictive information, the literature has largely focused on individual trades or firm-level aggregation. This study instead examines whether temporal clustering of purchases by multiple insiders conveys incremental information.

## 3. Data

Insider filing data was sourced from OpenInsider, which provides structured records of SEC Form 4 filings. I collected all filings classified as open-market purchases from January 2015 through December 2024. The raw dataset contained 92,396 filing records. After standardizing ticker symbols, parsing dates, filtering to open-market purchases only, applying a \$5 minimum transaction price floor, and removing duplicate filings (same ticker, same date, same insider name), the cleaned dataset contained 90,424 filings across 5,696 unique tickers.

Daily adjusted closing prices were downloaded from Yahoo Finance via the yfinance Python library for all tickers present in the cleaned filings, with a start date of June 2014 (to support pre-event estimation windows) and an end date of January 2025. The resulting price panel covers 2,665 trading days across 3,441 tickers. Of the 5,696 unique tickers in the cleaned filings, 2,255 (39.6%) could not be matched to downloadable price data and were excluded. These failures include delisted firms, renamed tickers, and data provider gaps. This is a known source of potential survivorship bias and is discussed in Section 7. SPY adjusted close prices were downloaded separately over the same period.

Market capitalizations were obtained from Yahoo Finance. These values reflect the most recent available figure at the time of data collection rather than the value at the time of each event, and therefore constitute a look-ahead measure. This bias in the universe filter is documented as a limitation in Section 7.
...
The primary result of this study — a mean 30-day cumulative abnormal return of +1.65% following clustered insider purchases — warrants discussion along four dimensions: economic magnitude, mechanism, tradability, and signal decay.

**Economic magnitude.** A +1.65% abnormal return over 30 trading days is economically meaningful at the individual event level. Naively annualized, this corresponds to approximately 1.65% × (252 / 30) ≈ 13.9% per year if an investor could continuously deploy capital into cluster events. However, this extrapolation overstates implementable returns because cluster events occur only approximately 266 times per year across 1,030 unique firms, and not all events can be traded simultaneously without portfolio concentration risk. For context, Jeng, Metrick, and Zeckhauser (2003) estimate abnormal returns of approximately 6% per year from calendar-time portfolios mimicking all insider purchases. The substantially higher per-event magnitude documented here is consistent with cluster events representing a filtered, higher-conviction subset of the broader insider purchase universe. Requiring 3 or more insiders to buy within a 30-day window selects for situations where private information is sufficiently compelling that multiple individuals act on it independently, raising the average signal quality relative to isolated individual insider transactions.

**Mechanism.** Why might clustered insider purchases be more informative than individual trades? Several non-exclusive explanations are consistent with the data. First, multiple insiders may independently observe favorable firm fundamentals — such as an improving order pipeline, cost reductions, or resolution of a pending uncertainty — and each conclude that the stock is undervalued. The temporal clustering of their purchases reflects convergent private assessments rather than coordinated action. Second, the cluster pattern may proxy for conviction concentration: when 3 or more insiders are willing to commit personal capital within a narrow window, the implied strength of the shared information signal is higher than when a single insider buys. This interpretation aligns with Cohen, Malloy, and Pomorski (2012), who show that opportunistic insider trades — those driven by information rather than routine portfolio rebalancing — are far more predictive of future returns. Cluster events may naturally filter toward opportunistic trades, since routine purchases by multiple insiders within the same month would be coincidental rather than systematic. It is important to note what cluster events are not: they do not necessarily indicate collusion, illegal coordination, or even mutual awareness among the insiders. Each insider files independently with the SEC, and the cluster pattern may emerge from independent reactions to the same underlying fundamentals.

**Tradability.** The sample contains 2,664 events over 10 years, or approximately 266 events per year — roughly one per trading day on average. The event date is the SEC filing date, which is publicly observable in real time via the EDGAR filing system. A trader monitoring Form 4 filings could identify cluster completions on the day they occur: when a third unique insider files a purchase for a given stock within 30 days of two prior filings, the cluster signal activates. However, the practical implementability of the signal is constrained by two factors. First, the CAR(0, 1) of +1.22% indicates that the majority of the short-horizon abnormal return is captured on day 0 itself (the filing date or next trading day), implying that execution speed is critical and that delayed entry would substantially reduce realized returns. Second, no transaction cost modeling is performed in this study. Round-trip trading costs, bid-ask spreads, and market impact — particularly for the smaller-cap stocks in the sample — could erode a meaningful portion of the gross abnormal return. Whether the signal supports a net-profitable trading strategy remains an open question for future research.

**Signal decay.** The time-path of abnormal returns provides additional insight into the information incorporation process. Of the +1.65% mean CAR at 30 days, +1.22% is realized by the end of day 1, and +1.47% by the end of day 5. This front-loading suggests that the market responds rapidly to the public disclosure of insider filings, with the bulk of the price adjustment occurring within the first trading session. The continued drift from day 5 to day 60 (+3.23% total CAR) may reflect slower diffusion of the underlying information to less attentive market participants, gradual fundamental realization (e.g., subsequent earnings surprises), or a combination of both. However, the widening confidence intervals at longer horizons indicate that this extended drift is noisier and less reliable at the individual event level.
...
Clustered insider open-market purchases — defined as 3+ unique insiders filing purchases for the same stock within 30 calendar days — are associated with positive and statistically significant abnormal returns in U.S. equities over the 2015–2024 period. The primary pre-registered test shows a mean 30-day cumulative abnormal return of +1.65% (t = 5.42, p = 6.6e-08). The result is stable across time subperiods, robust to CAPM risk adjustment, and not sensitive to moderate changes in signal parameters.

As discussed in Section 7, these findings are consistent with the hypothesis that coordinated insider purchasing conveys stronger information than individual insider transactions. The per-event magnitude substantially exceeds the ~6% annual return estimated by Jeng et al. (2003) for general insider purchases, supporting the interpretation that cluster events filter for high-conviction insider behavior. The pre-registered design ensures that the result is not an artifact of specification search.
